/**
 * @file sniff_karma_menu_screen.c
 * @brief WiFi Sniff & Karma sub-menu screen implementation
 */
...
static const char *TAG = "SNIFF_KARMA";
...
typedef struct {
    int selected_index;
} sniff_karma_menu_data_t;

static void draw_screen(screen_t *self)
{
    sniff_karma_menu_data_t *data = (sniff_karma_menu_data_t *)self->user_data;
    
    ui_clear();
    
    // Draw title
    ui_draw_title("WiFi Sniff & Karma");
...
static void redraw_selection(sniff_karma_menu_data_t *data, int old_index, int new_index)
{
        ...
        case KEY_SPACE:
            {
                switch (data->selected_index) {
                    case 0:  // Sniffer
                        screen_manager_push(global_sniffer_screen_create, NULL);
                        break;
                    case 1:  // Browse Clients
                    ...
                    case 2:  // Show Probes
                        ...
                        break;
                    case 3:  // Karma
                        screen_manager_push(karma_probes_screen_create, NULL);
                        break;
                }
            }
            break;
...
screen_t* sniff_karma_menu_screen_create(void *params)
{
    (void)params;
    
    ESP_LOGI(TAG, "Creating sniff karma menu screen...");
    
    screen_t *screen = screen_alloc();
    if (!screen) return NULL;
    
    // Allocate user data
    sniff_karma_menu_data_t *data = calloc(1, sizeof(sniff_karma_menu_data_t));
    if (!data) {
        free(screen);
        return NULL;
    }
    ...
    ESP_LOGI(TAG, "Sniff karma menu screen created");
    return screen;
}

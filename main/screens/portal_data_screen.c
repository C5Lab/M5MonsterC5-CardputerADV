...
static void draw_title(portal_data_data_t *data)
{
    char title[32];
...
static void draw_entry_row(portal_data_data_t *data, int entry_idx)
{
...
static void draw_content(portal_data_data_t *data)
{
    ...
    if (data->loading) {
        ...
        ui_print_center(3,
                        data->status_message[0] ? data->status_message : "No portal data found",
                        UI_COLOR_DIMMED);
    } else {
        for (int i = 0; i < VISIBLE_ITEMS; i++) {
...
static void uart_line_callback(const char *line, void *user_data)
{
    ...
    if (strstr(line, "not found") != NULL || strstr(line, "Not found") != NULL ||
        ...
        data->loading = false;
        if (strstr(line, "not found") != NULL || strstr(line, "Not found") != NULL) {
            snprintf(data->status_message, sizeof(data->status_message), "portals.txt not found");
        } else {
